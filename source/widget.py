...
class Widget(QWidget):
    ...
    def init_ui(self):
        ...
        self.order_func_hbox = self.create_hbox(('P', 'C'), key='(')
        ...
        self.hbox[0].addWidget(Button(text='Ran', master=self, key='('))
        self.hbox[0].addLayout(self.order_func_hbox[0])
    ...
    def change_trig_mode(self):
        if evaluator.trig_mode == TrigMode.RADIAN:
            evaluator.trig_mode = TrigMode.DEGREE
            self.change_trig_mode_button.setText('Deg')
        else:
            evaluator.trig_mode = TrigMode.RADIAN
    ...
    def solve_equation(self):
        ...
        p.setX(950)
        p.setY(200)
        SolveEquationWidget.run(position=p)

    def evaluate(self):
        self.eval = True
        t = self.entry.text()
        self.result = str(round(evaluator.parse(t), self.round_num))
        self.entry.clear()
